...
@Component({
  selector: 'app-editallocate',
  templateUrl: './editallocate.component.html',
  styleUrls: ['./editallocate.component.css']
})
export class EditallocateComponent {
  OrgID: any;
...
WorkingDays:Array<TextValue>=[];
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private globalToastService: ToastrService, private _commonservice: HttpCommonService, private toastr: ToastrService,
    private spinnerService: NgxSpinnerService, public dialogRef: MatDialogRef<EditallocateComponent>,private dialog:MatDialog
  ) {
    this.formInput={ StartDate:'' }
    this.isEdit = this.data.isEdit || false;
    this.branch = this.data.row?.Branch || '';
    this.department = this.data.row?.Department.Name || 'No Department';
    this.EmployeeName = this.data.row?.EmployeeName || '';
    this.IsWholeYear = this.data.row?.IsWholeYear || '';
    this.formInput.StartDate=this.convertDate(this.data.row?.StartDate)||''
  ...
  updateAllocation() {
    ...
    var shiftselected= this.DynamicArray.filter((item:any) => item.ShiftID!=0);
    ...
    {
if(this.DynamicArray[i].ShiftID>0)
{
  var checkday= this.DynamicArray[i].WorkingDays.filter((item:any) => item.Value ===true);
  if(checkday==null)
  {
    WorkDayMissingmessage="Please Select working days for Shift "+i; 
    break;
  }
}
    }
