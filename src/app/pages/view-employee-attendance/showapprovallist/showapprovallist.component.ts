...
export class ShowapprovallistComponent {
  ...
  private spinnerService: NgxSpinnerService,private toastr: ToastrService,private dialog:MatDialog,
  ...
  GetList() {
    ...
    for( let index=0;index<this.Employees.length;index++)
    {
        ...
        tmp.push({"Empid":this.Employees[index].EmployeeID,"Time":this.Date })
    }
  ...
  ApproveAll() {
      this._commonservice.ApiUsingPost("Admin/ApproveAllAttendance",this.AttendanceList).subscribe(data => {
        if (data.Status == true) {
