...
@Component({
  selector: 'app-common-paginated-table',
  templateUrl: './common-paginated-table.component.html',
  styleUrls: ['./common-paginated-table.component.css']
})
export class CommonPaginatedTableComponent  implements OnInit,OnChanges {
  ...
  @Input() scrollPagination:any
  @Output() StatusUpdate: EventEmitter<any> = new EventEmitter()
  @Output() actionEmitter: EventEmitter<any> = new EventEmitter()
  @Output() onPageNumberChange: EventEmitter<any> = new EventEmitter()
  @Output() onPageEnd: EventEmitter<any> = new EventEmitter()

  
  pageSize:any = 10
  totalRecords:any
  pageIndex:any = 0
  LoadingPagination:any = false
  ...
  setTableScrolling() {
    setTimeout(() => {
      ...
      container.addEventListener('scroll', () => {
        ...
        const threshold = 50; // pixel buffer from exact bottom
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if(changes['Loading']){
      if(this.scrollPagination == true ) this.setTableScrolling()
    }
    if(changes['data']?.currentValue){
      this.dataSource.data = changes['data'].currentValue
      // if(!this.paginatedData){
      //   console.log("setting totalRecords");
      //   this.totalRecords = this.dataSource?.data?.length || 0
      // }
    ...
    if(changes['paginatedData']?.currentValue){
      this.dataSource.paginator = this.paginator
      console.log(this.paginatedData,changes['paginatedData']);
      this.setPagination()
    }
  }
  ...
  setPagination(){
    ...
    this.Loading = true
    this.pageSize = this.paginatedData.limit
    this.totalRecords = this.paginatedData.totalRecord
    this.pageIndex = this.paginatedData.currentPage-1
    
    setTimeout(() => {
      this.Loading = false
      this.LoadingPagination = false
    }, 100);
    console.log("setting ",this.pageSize, this.totalRecords, this.pageIndex, this.LoadingPagination);
  }
  
  applyFilter() {

    // this.dataSource.filter = this.searchText.trim().toLowerCase();
    this.actionEmitter.emit({row:this.dataSource,action:{name:"searchUpdate"}, searchText:this.searchText})
  }
  ...
  isDateType(data: any,column:any): any {
    let validDateHeader:any = ["LoanIssuedDate","EmplooyeeCreatedDate","EmployeeDeletedDate","CreatedDate","Date","AssignedDate","ModifiedDate","Startdate",
      "assignedDate","returnDate","uploadedDate","createdDate",
      "createdAt","uploadedAt","updatedDate","approvedDate"]
  ...
  isDateTimeType(data: any,column:any): any {
    let validDateHeader:any = ['CreatedDateTime']
    if(validDateHeader.includes(column)){
      const date = new Date(data);
      if(date.getFullYear() == 1970) return false
      return !isNaN(date.getTime()); // Check if the date is valid
  ...
  onRowCheckboxChange(row: any) {
    const index = this.selectedRows.findIndex((selectedRow) => selectedRow.EmployeeID === row.EmployeeID);
  ...
  exportPdf(data:any,selectedHeader:any) {
    ...
    // Center ExportName
    const textWidthExportName = doc.getTextWidth(this.ExportName);
    doc.text(this.ExportName, (pageWidth - textWidthExportName) / 2, 20);
    // if(this.ExportBranch){
    //   doc.text(this.ExportBranch,200, 30);
    // }
    // if(this.ExportYearMonth){
    //   doc.text(this.ExportYearMonth,220, 40);
    // }
    if (this.ExportYearMonth) {
